...
static cl_int queue_mtp_kernel_new(_clState *clState, dev_blk_ctx *blk, __maybe_unused cl_uint threads)
{
	struct pool *pool = blk->work->pool;
	mtp_cache_t *mtp = &blk->work->thr->cgpu->mtp_buffer.mtp_cache;
	size_t worksize = clState->wsize;
...
for (int i=0;i<4;i++) {
		...
		status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock, true, TheOffSet + TheOffSet1 * 0, TheSize, (uchar*)(mtp->instance.memory[0].v + ThatSize), 0, NULL, NULL);
		if (status != CL_SUCCESS)
			applog(LOG_ERR, "problem copying instance to hblock", status);

		status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock, true, TheOffSet + TheOffSet1 * 1, TheSize, (uchar*)(mtp->instance.memory[1].v + ThatSize), 0, NULL, NULL);
		if (status != CL_SUCCESS)
			applog(LOG_ERR, "problem copying instance to hblock", status);

		status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock, true, TheOffSet + TheOffSet1 * 1048576, TheSize, (uchar*)(mtp->instance.memory[2].v + ThatSize), 0, NULL, NULL);
		if (status != CL_SUCCESS)
			applog(LOG_ERR, "problem copying instance to hblock", status);

		status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock, true, TheOffSet + TheOffSet1 * 1048577, TheSize, (uchar*)(mtp->instance.memory[3].v + ThatSize), 0, NULL, NULL);
		if (status != CL_SUCCESS)
			applog(LOG_ERR, "problem copying instance to hblock", status);

		status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock, true, TheOffSet + TheOffSet1 * 2097152, TheSize, (uchar*)(mtp->instance.memory[4].v + ThatSize), 0, NULL, NULL);
		if (status != CL_SUCCESS)
			applog(LOG_ERR, "problem copying instance to hblock", status);

		status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock, true, TheOffSet + TheOffSet1 * 2097153, TheSize, (uchar*)(mtp->instance.memory[5].v + ThatSize), 0, NULL, NULL);
		if (status != CL_SUCCESS)
			applog(LOG_ERR, "problem copying instance to hblock", status);

		status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock, true, TheOffSet + TheOffSet1 * 3145728, TheSize, (uchar*)(mtp->instance.memory[6].v + ThatSize), 0, NULL, NULL);
		if (status != CL_SUCCESS)
			applog(LOG_ERR, "problem copying instance to hblock", status);

		status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock, true, TheOffSet + TheOffSet1 * 3145729, TheSize, (uchar*)(mtp->instance.memory[7].v + ThatSize), 0, NULL, NULL);
		if (status != CL_SUCCESS)
			applog(LOG_ERR, "problem copying instance to hblock", status);
}
for (int i = 0; i<4; i++) {
	...
	status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock2, true, TheOffSet + TheOffSet1 * 0, TheSize, (uchar*)(mtp->instance.memory[0].v + ThatSize), 0, NULL, NULL);
	if (status != CL_SUCCESS)
		applog(LOG_ERR, "problem copying instance to hblock2", status);

	status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock2, true, TheOffSet + TheOffSet1 * 1, TheSize, (uchar*)(mtp->instance.memory[1].v + ThatSize), 0, NULL, NULL);
	if (status != CL_SUCCESS)
		applog(LOG_ERR, "problem copying instance to hblock2", status);

	status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock2, true, TheOffSet + TheOffSet1 * 1048576, TheSize, (uchar*)(mtp->instance.memory[2].v + ThatSize), 0, NULL, NULL);
	if (status != CL_SUCCESS)
		applog(LOG_ERR, "problem copying instance to hblock2", status);

	status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock2, true, TheOffSet + TheOffSet1 * 1048577, TheSize, (uchar*)(mtp->instance.memory[3].v + ThatSize), 0, NULL, NULL);
	if (status != CL_SUCCESS)
		applog(LOG_ERR, "problem copying instance to hblock2", status);

	status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock2, true, TheOffSet + TheOffSet1 * 2097152, TheSize, (uchar*)(mtp->instance.memory[4].v + ThatSize), 0, NULL, NULL);
	if (status != CL_SUCCESS)
		applog(LOG_ERR, "problem copying instance to hblock2", status);

	status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock2, true, TheOffSet + TheOffSet1 * 2097153, TheSize, (uchar*)(mtp->instance.memory[5].v + ThatSize), 0, NULL, NULL);
	if (status != CL_SUCCESS)
		applog(LOG_ERR, "problem copying instance to hblock2", status);

	status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock2, true, TheOffSet + TheOffSet1 * 3145728, TheSize, (uchar*)(mtp->instance.memory[6].v + ThatSize), 0, NULL, NULL);
	if (status != CL_SUCCESS)
		applog(LOG_ERR, "problem copying instance to hblock2", status);

	status |= clEnqueueWriteBuffer(clState->commandQueue, buffer->hblock2, true, TheOffSet + TheOffSet1 * 3145729, TheSize, (uchar*)(mtp->instance.memory[7].v + ThatSize), 0, NULL, NULL);
	if (status != CL_SUCCESS)
		applog(LOG_ERR, "problem copying instance to hblock2", status);
}
...
#undef A_DARK
  ...
  { "mtp-tcr"    , ALGO_MTP   , "", 1, 1, 1, 0, 0, 0xFF, 0xFFFFFFFFULL, 0x0000ffffUL, 1, 0,0, mtp_regenhash   , NULL, queue_mtp_kernel   , gen_hash, NULL },
