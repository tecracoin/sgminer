...
static void gen_stratum_work(struct pool *pool, struct work *work)
{
  ...
  * from left to right and prevent overflow errors with small n2sizes */
  ...
  memcpy(pool->coinbase + pool->nonce2_offset, &nonce2le, pool->n2size);
/*
  printf("coming here before prev_job_id stratum\n");
  printf("job_id %s\n", pool->swork.job_id);
  printf("prev_job_id %s\n", pool->swork.prev_job_id);
*/
/*
	if (pool->algorithm.type==ALGO_MTP) {
	  if (pool->swork.prev_job_id)
		if (strcmp(pool->swork.prev_job_id, pool->swork.job_id)) {
				work->nonce2 = pool->nonce2++;
...
*/
  ...
  if (pool->algorithm.type == ALGO_NEOSCRYPT) {
    ...
    * into work. */
    ...
    ((uint32_t *)work->data)[31] = 0x00000280;
	  ...
	  * into work. 
	  for (i = 0; i < temp; ++i) {
		  ((uint32_t *)work->data)[i] = le32toh(((uint32_t *)pool->header_bin)[i]);
	  }
  ...
  * stratum diff when submitting shares */
  ...
  /* Copy parameters required for share submission */
  work->job_id = strdup(pool->swork.job_id);
//  work->prev_job_id = strdup(pool->swork.prev_job_id);
  work->nonce1 = strdup(pool->nonce1);
  work->ntime = strdup(pool->swork.ntime);
  cg_runlock(&pool->data_lock);
